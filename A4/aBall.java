/**
 * aBall class code of the 4th assignment for ECSE 202 with prof. Ferrie, Fall 2019.
...
public class aBall extends Thread{	
	...
	public static double TICK = 0.01; 				// Clock tick duration (sec) #
	...
	/**
	 * trace method
	 * adds trace points to x and y coordinates
	 */
	private void trace(double x, double y) {
		double ScrX = x*SCALE;
		double ScrY = HEIGHT - y*SCALE;
		GOval pt = new GOval(ScrX,ScrY,1,1);
	...
	*/
	...
	public void run()
	{
	    ...
	    while(hasEnergy)
	    {
		    ...
		    // if link is provided put trace points #
		    if (link!=null)
		    {
		    	trace(X+X0, Y);
		    }
		    ...
		    try { 																	// pause for 50 milliseconds #
		    	Thread.sleep((long) (TICKmS/2));									// for actual time change 50 to 2
